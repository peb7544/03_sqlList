...
-- SELECT 조회 결과 집합 인 RESULT SET에 출력되는 컬럼명을 지정
...
-- NOT BETWWEN A AND B : A 이상 B 이하가 아닌 경우 --> A 미만, B 초과
...
-- EMPLOYEE 테이블에서 입사일이 1990-01-01 ~ 1991-12-31 사아인 직원의
-- 이름, 입사일 조회

SELECT EMP_NAME, HIRE_DATE
FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN '1990-01-01' AND '1999-12-31';
-- 오라클은 데이터 타입이 달라도 형태가 일치하면 자동으로 변환시킴
...
-- EMPLOYEE 테이블에서 성이 '전'씨 사원의 사번, 이름 조회
 ...
 * 1. FROM절
 * 2. WHERE절
 * 4. ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [ASC | DESC] [NULL FIRST | LAST]
 *                            (오름차순:기본값) (내림차순)
 * */
