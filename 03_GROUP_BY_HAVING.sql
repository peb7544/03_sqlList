 ...
 * 6 : ORDER BY 컬럼명 | QUFCLD | 컬럼순번 정렬방식(ASC/DESC) [NULL FIRST | LAST]
 */

--------------------------------------------------------------------------------------------------------------------

-- * GROUP BY절 : 같은 값들이 여러개 기록된 컬럼을 가지고 같은값들을
--                하나의 그룹으로 묶음
...
-- 여러 개의 결과값을 산출하기 위해 그룹함수가 적용된 그룹의 기준을 ORDERBY절에 기술하여 사용
...
ORDER BY "인원 수"; -- 별칭사용O(SELECT 절 해석 완료)
...
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE 그룹 중 급여 평균이 3백만 이상인그룹만 조회
ORDER BY DEPT_CODE;
...
-- UNION ALL : UNILON + INTERSECT
--             합집합에서 중복 부분 제거 x

-- MINUS(차집합) : A에서 A,B 교집합 부분을제거하고 조회
