...
-- 함수는 SELECT문의
-- SELECT 절, WHERE 절, ORDER BY절, GROUP BY 절, HAVING절 사용가능
...
-- 문자열을 5번째 문자부터 검색하여 두 번재 B 위치 조회
SELECT INSTR('AABAACAABBAA', 'B',5,2) FROM DUAL;

-- EMPLOYEE 테이블에서 사원명, 이메일, 이메일 중 '@' 위치 조회
SELECT 
	EMP_NAME, EMAIL, INSTR(EMAIL, '@')
FROM EMPLOYEE;

------------------------------------------------------

-- SUBSTR(컬럼명 || 문자열, 잘라내기 시작할 위치 [, 잘라낼 길이])
-- 컴럼이나 문자열에서 지정한 위치부터 지정된 길이만큼 문자열을 잘라내서 반환
--> 잘라낼 길이 생략 시 끝까지 잘라냄

-- EMPLOYEE 테이블에서 사원명, 이메일 중 아이디 부분만 조회
SELECT
	EMP_NAME, EMAIL, SUBSTR(EMAIL, 1, INSTR(EMAIL, '@')-1) 아이디
FROM EMPLOYEE;
...
SELECT 
	EMP_NAME , SALARY , MOD(SALARY, 1000000)
FROM EMPLOYEE;
...
SELECT ROUND(123.456, 1) FROM DUAL; -- 123, 소수점 두 번째 자리에서 반올림
                                    --> 두 번재 자리에서 반올림해서
                                    -- 소수점 한 자리만 표험
...
-- SYSDATE : 시스템의 현재 시간을 반환

SELECT SYSDATE FROM DUAL;

-- SYSTIMESTAMP : SYSDATE + MS 단위 추가
SELECT  SYSTIMESTAMP  FROM DUAL;
-- TIMESTAMP : 특정 시간을 나탄내거나 기록하기 위한 문자열
-- 2024-02-29 11:21:23.622 +0900 (UTC에서 +9시간 나타냄, 한국 표준시)

-- MONTHS_BETWEEN(날짜, 날짜) : 두 날짜의 개월 수 차이 반환
SELECT 
	ROUND(MONTHS_BETWEEN(SYSDATE, '2024-06-26'), 3) "수강 기간(개월)" 
FROM DUAL; 
...
SELECT TO_CHAR( EXTRACT(MONTH FROM HIRE_DATE), '00') FROM EMPLOYEE;
...
SELECT SYSDATE  FROM DUAL; -- 2024-02-29 12:21:45.000
...
SELECT 
	EMP_NAME, 
	TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, INSTR(EMP_NO, '-')-1)), 'YYYY"년" MM"월" DD"일"') AS 생년월일
FROM EMPLOYEE;
...
SELECT 
	EMP_NAME, NVL2(BONUS, 'O', 'X') "보너스 수령"
FROM EMPLOYEE;
...
-- 비교하고자 하는 값 또는 컬럼이 조건식과 같으면 결과 값 변환
