...
--> 위의 2단계를 하나의 쿼리를 만든다 --> 1번 쿼리를 서브쿼리로
SELECT
	 EMP_ID
	,EMP_NAME 
	,JOB_CODE 
 ...
 * 
 * - 단중행(단일열) 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 여러개일 때
 * 
 * - 다중열 서브쿼리 : 서브쿼리의 SELECT 절에 나열된 항목수가 여러개일 때
 * 
 * - 다중행 다중열 서브쿼리 : 조회 결과 행 수와 열 수가 여러개일 때
 * 
 * - 상(호연)관 서브쿼리 : 서브쿼리가 만든 결과 값을 메인쿼리가 비교 연산할 대
...
-- 4) 일반 직원에 해당하는 사원들 정보 조회(구분은 '사원'로)
SELECT 
	 EMP_ID
	,EMP_NAME 
	,DEPT_TITLE
...
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
WHERE EMP_ID NOT IN (SELECT DISTINCT 
										MANAGER_ID 
									FROM EMPLOYEE
									WHERE MANAGER_ID IS NOT NULL);	
									
-- 5) 3, 4의 조회 결과를 하나로 합칩 -> SELECT 절 SUBQUERY
-- * SELECT 절에도 서브쿼리 사용할 수 있다.
								
-- 선택함수 사용!
--> DECODE(컬럼값, 값1, 1인 경우, 값2, 2인 경우,.. 일치하지 않는 경우)
...
SELECT
	...
	,DEPT_TITLE
	,JOB_NAME
	,CASE WHEN EMP_ID IN (SELECT DISTINCT 
													MANAGER_ID 
												FROM EMPLOYEE
												WHERE MANAGER_ID IS NOT NULL)	
						THEN '사수'
...
-- 1) 직급인 대리인 직원들의 사번, 이름, 직급, 급여를 조회
SELECT 
	 EMP_ID
	,EMP_NAME 
	,JOB_NAME
...
-- DEPARTMENT 테이블의 LOCTION_ID와 동일한 DEPT_ID와
...
-- 3) 최종적으로 EMPLOYEE테이블에서 위의 결과들과 동일한 DEPT_CODE와 동일한 사원을 구하시오.
SELECT 
	 EMP_NAME
	,DEPT_CODE 
FROM EMPLOYEE
...
SELECT
	...
	,HIRE_DATE 
FROM EMPLOYEE
WHERE (DEPT_CODE, MANAGER_ID) = (SELECT
																 	  DEPT_CODE 
																 	 ,MANAGER_ID 
																 FROM EMPLOYEE
...
/* IN 서브쿼리는 메인쿼리의 조건ㅇ르 만족하는 데이터를 반환함
 * 따라서 각 지원의 MANAGER_ID가 다른 직원의 EMP_IN와 일차흐는 경우에
 * 해당 직원의 정보를 반환함
 * */

-- 방법 2) 
-- EXITS : 서브쿼리에서 조회된 결과와 일치하는 행이
--         메인쿼리에 하나라도 있으면 조회 결과에 포함됨
SELECT 
	 EMP_ID 
	,EMP_NAME 
	,DEPT_TITLE
 ...
 * 실제 데이터 값을 반환하지 않음
 * 즉, 각 지원의 MANAGER_ID가 다른 직원의 EMP_ID와 일치하는 경우만 확인됨
 * 
...
-- 메인쿼리
-- 사번, 이름, 부서명(NULL이면 '소속없음', 직급명, 입사일 조회						
-- 단, 퇴사한 직원은 제외
SELECT 
	 EMP_ID 
	,EMP_NAME 
...
/*
 * 214	방명수	인사관리부	사원	2010-04-04 00:00:00.000
 * 216	차태연	인사관리부	대리	2013-03-01 00:00:00.000
 * 217	전지연	인사관리부	대리	2007-03-20 00:00:00.000
 * */
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D1';
/* 1. 메인쿼리의 첫번째 행인 방명수가 있는 부서 01의 가장 빠른 입사일을 조회했는데,
 *    2007-03-20 00:00:00.000
 *    방명수 입사일이랑 다름. 방명수는 아님!(RESULT SET 제외)
 * 2. 차태연 입사일이랑 다름. 차태연 아님!(RESULT SET 제외)
 * 3. 전지연 입사일이랑 같음. 전지연이 'D1' 부서의 가장 빠른 입사자(RESULT SET 포함)
 * */
...
-- DENSE_RANK() OVER : 동일한 순위 이후의 등수를 이후의 순서로 계산
-- EX) 공동 1위가 2명이도 다음 순위 2위
SELECT 
