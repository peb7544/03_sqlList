...
ALTER TABLE DEPT_COPY 
ANDD CONSTRAINT DEPT_COPY_TITLE_NM NOT NULL(DEPT_TITLE);
...
-- ORA-12899: "KH_PEB"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)

-- DEPT_ID 컬럼 데이터 타입 수정
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR(3);

-- LNAME의 기본값을 'KOREA'로수정
...
-- 3) DROP TABLE 삭제옵션 CASCADE CONSTRAINTS 사용
   --> CASCADE CONSTRAINS : 삭제하려는 테이블과 연결된 FK 제약조건을 모두 삭제
...
--> DDL은 수행 시 존재하고 있는 트랜젝션을 모두 DB에 강제 COMMIT 시킴
