...
-- 2. 춘 기술대학교의 교수 중 이름이 세 글자가 아닌 교수가 두 명 있다고 한다. 그 교수의
...
SELECT 
	...
	/*CASE WHEN TO_CHAR(SYSDATE, 'mmDD') > LTRIM(SUBSTR(PROFESSOR_SSN, 3, 4), 0)
		   THEN TO_CHAR(SYSDATE, 'YYYY') - TO_CHAR(19 || SUBSTR(PROFESSOR_SSN, 1, 2)) -2
		   ELSE TO_CHAR(SYSDATE, 'YYYY') - TO_CHAR(19 || SUBSTR(PROFESSOR_SSN, 1, 2)) -1 
		   END 나이2*/
FROM TB_PROFESSOR
...
-- 5 춘 기술대학교의 재수생 입학자를 구하려고 한다. 어떻게 찾아낼 것인가? 이 때,
...
SELECT
	TO_CHAR(TO_DATE('99/10/11','YY/MM/DD'),'YYYY"년" MM"월" DD"일"'), -- 2099년 10월 11일
	TO_CHAR(TO_DATE('49/10/11','YY/MM/DD'),'YYYY"년" MM"월" DD"일"'), -- 2049년 10월 11일
	TO_CHAR(TO_DATE('99/10/11','RR/MM/DD'),'YYYY"년" MM"월" DD"일"'), -- 1999년 10월 11일
	TO_CHAR(TO_DATE('49/10/11','RR/MM/DD'),'YYYY"년" MM"월" DD"일"')  -- 2049년 10월 11일
FROM DUAL;

-- 8 춘 기술대학교의 2000년도 이후 입학자들은 학번이 A로 시작하게 되었다. 2000년도
...
--   이 때  출력 화면의 헤더는 "평점"이라고 찍히게 하고, 점수는 반올림하여 소수점 이하 한
--   자리까지만 표시한다.
...
-- 13 학과 별 휴학생 수를 파악하고자 한다. 학과 번호와 유학생 수를 표시하는 SQL 문장을
--    작성하시오.
...
-- 15 학번이 A112113인 김고운 학생의 년도, 학기 별 평점과 년도 별 누적 평점, 총
