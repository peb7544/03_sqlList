...
INSERT INTO MEMBER VALUES
('MEM02', 'QWER123', '김영희', '970506-2233445', SYSDATE);
...
INSERT INTO "MEMBER" VALUES 
('MEM04', 'PASS04', '김길동', '930303-1333333', TO_DATE('2022-09-13 17:33:27', 'YYYY-mm-DD HH24:MI:SS'));
...
--> MEMBER 타입 컬럼에 데이터 삽입시
-- 제일 앞에 0DL DLtDMAUS DLFMF WKEHDDMFH WPRJGKA
   --> 전화번호, 주민등록번호처럼 숫자로만 되어있는 데이터지만
   --> 0으로만 시작할 가능성이 있으면 CHAR, VARCHAR2 같은 문자형을 사용
...
--SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH_PEB"."USER_USED_NM"."USER_NO") 안에 삽입할 수 없습니다
...
INSERT INTO USER_USED_UK 
VALUES(1, 'USER01', 'PASS01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_PEB.USER_ID_U)에 위배됩니다
...
-- * 복합키 지정은 테이블 레멜만 가능하다! *
-- * 봅합키는 지정된 모든 컬럼의 값이 같을 때 위배된다! *
...
-- ORA-00001: 무결성 제약 조건(KH_PEB.USER_ID_NAME_U)에 위배됩니다.
...
-- 기본키 중복 오류
-- ORA-00001: 무결성 제약 조건(KH_PEB.USER_NO_PK)에 위배됩니다.

INSERT INTO USER_USED_PK 
VALUES(NULL, 'USER03', 'PASS03', '유관순', '여', '010-9999-5678', 'yoo123@kh.or.kr');
-- 기본키 NULL이므로 오류
-- ORA-01400: NULL을 ("KH_PEB"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(KH_PEB.PK_USERNO_USERID)에 위배됩니다
-- 회원번호와 아이디 둘 다 중복되었을때만 제약조건 위배 에러 발생

INSERT INTO USER_USED_PK2
VALUES(NULL, 'USER01', 'PASS02', '신사임당', '여', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-01400: NULL을 ("KH_PEB"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
-- PRIMARY KEY NULL이 들어갈 수 없음
...
-- 참조(REFERENCES)된 다른 테이블이 컬럼이 제공하는 값만 사용할 수 있음
-- FOREING KEY 제약조건에 의해서 테이블간의 관계가 형성됨
-- 제공되는 값 외에는 NULL을 사용할 수 있음

-- 컬럼레벨일 경우
-- 컬럼명 자료형(크기) [CONSTRAINT 이름] REFEREBCES 참조할 테이블명 [(참조할컬럼)] [삭제룰]

-- 테이블레벨일 경우
-- [CONSTRAINT 이름] FOREING KEY (적용할컬럼명) REFERENCES 참조할테이블명
...
CREATE TABLE USER_GRADE(
	...
	GRADE_NAME VARCHAR(30) NOT NULL -- 등급명
);
...
CREATE TABLE USER_USED_FK(
	...
	-- 테이블 레벨
	-- , CONTRAINT GRADE_CODE_FK FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE
...
INSERT INTO USER_USED_FK
VALUES(4, 'USER04', 'PASS04', '안중근', '남', '010-2222-1111', 'ahn23@kh.or.kr',NULL);
--> NULL 사용 가능

INSERT INTO USER_USED_FK
VALUES(5, 'USER05', 'PASS05', '윤봉길', '남', '010-2222-1111', 'ahn23@kh.or.kr',50);
--> ORA-02291: 무결성 제약조건(KH_PEB.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
--> 50이라는 값은 USER_GRADE 테이블 FRADE_CODE 컬럼에서 제공하는 값이 아니므로
...
-- ORA-02292: 무결성 제약조건(KH_PEB.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- 부모테이블인 USER-GRADE2에서 GRADE_CODE = 10 삭제
...
-- ORA-02290: 체크 제약조건(KH_PEB.GENDER_CHECK)이 위배되었습니다

---------------------------------------------------------------------------------------

-- 8. SUBQUERY를 이용한 테이블 생성
-- 컬렴명, 데이터 타입, 값이 복사되고, 제약조건은 NOT NULL만 복사
...
-- ALTER TABLE 테이블명 ADD [CONTRAINT 제약조건명] UIQUE(컬럼명)
