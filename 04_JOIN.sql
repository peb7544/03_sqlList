...
--> (== 일치하는 값이 없는 행은 조인해서 제외됨)

-- 작성 방법은 크게 ANSI 구문과 오라클 구문으로 나뉘고
-- ANSI에서 UNING과 ON을 쓰는 방법으로 나뉜다.
...
-- 두 테이블의 지정하는 컬럼값이 일치하는 행도 조인에 포함 시킴
--> * 반드시 OUTER JOIN 명시해야 한다.

-- OUTER JOIN과 INNER JOIN 쿼리문
...
-- 3) FULL [OUTER] JOIN : 합치기에 사용한 두 테이블 중
--                        모든 행을 결과에 포함

-- ** 오라클 구문 FULL OUTER JOIN을 사용못함
...
-- '='(등호)를 사용하지 않는 조인문
-- 지정한 값이 일치하는 경우가 아닌, 값의 범위에 포함되는 행들을 연결하는 방식
